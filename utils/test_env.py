...
class ObservationSpace(object):
    def __init__(self, shape):
        self.shape = shape
        self.bad_state = np.random.randint(0, 100, shape, dtype=np.uint8)
        self.normal_state = np.random.randint(50, 150, shape, dtype=np.uint8)
        self.good_state = np.random.randint(100, 250, shape, dtype=np.uint8)
